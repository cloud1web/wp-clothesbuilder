...
function tshirtecommerce_quick_add_cart()
{
	...
	if(count($data))
	{
		...
		{
			$html = '<svg  width="500" height="500" viewBox="0 0 500 500" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" preserveAspectRatio="none">';
			if(count($items)> 0)
			{
				foreach($items as $item)
				{
					if( isset($item['svg']) )
					{
						//$svg = preg_replace('/<style>(.*)<\/style>/i', "", $item['svg']);
						$svg = $item['svg'];
						$svg = str_replace('\\"', '"', $svg);
						...
						if (isset($links[1][0]))
						{
							if (strpos($links[1][0], 'image/PNG;base64') === false)
							{
