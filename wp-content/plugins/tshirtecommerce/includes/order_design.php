...
add_action('tshirtecommerce_setting', 'design_order_detail_setting');
function design_order_detail_setting($values)
{
	...
	echo '<tr valign="top">'
			.'<th scope="row" class="titledesc">Download file design:</th>'
			.'<td class="forminp-text">';
	if (isset($values['allow_download']) && $values['allow_download'] == 1)
	{
		echo '<input type="checkbox" value="1" name="designer[allow_download]" checked="checked"> Allow client download file design.';
	}
	else
	{
		echo '<input type="checkbox" value="1" name="designer[allow_download]"> Allow client download file design.';
	}	
	echo '</td></tr>';
	
	echo '<tr valign="top">'
			.'<th scope="row" class="titledesc">Redirect to page cart:</th>'
			.'<td class="forminp-text">';
	if (isset($values['redirect_cart']) && $values['redirect_cart'] == 1)
	{
		echo '<input type="checkbox" value="1" name="designer[redirect_cart]" checked="checked"> Auto redirect to page cart after client click "Buy Now".';
	}
	else
	{
		echo '<input type="checkbox" value="1" name="designer[redirect_cart]"> Auto redirect to page cart after client click "Buy Now".';
	}	
	echo '</td></tr>';
}
		...
		$images = json_decode(str_replace('\\', '', $data['images']));
			if (count($images))
			{
				...
				foreach($images as $view => $image)
				{
					...
					if ($download == true)
					{
						echo '<br /><a target="_blank" href="'.site_url('tshirtecommerce/design.php?key='.$data['design_id'].'&view='.$view).'">'.$lang['order_download_design'].'</a>';
					}
					echo '</td>';
				}
				echo '</tr></table>';
	...
	else
	{
		...
		if($download && $product_id != '')
		{
			...
			echo '<p>'
					...
					.' <a href="'.site_url('tshirtecommerce/design.php?idea=1&key='.$product_id).'&view=front" target="_blank" title="'.$lang['order_click_to_view_design'].'">'.$lang['front'].'<strong></strong></a> - '
				.' <a href="'.site_url('tshirtecommerce/design.php?idea=1&key='.$product_id).'&view=back" target="_blank" title="'.$lang['order_click_to_view_design'].'"><strong>'.$lang['back'].'</strong></a> - '
					.' <a href="'.site_url('tshirtecommerce/design.php?idea=1&key='.$product_id).'&view=left" target="_blank" title="'.$lang['order_click_to_view_design'].'"><strong>'.$lang['left'].'</strong></a> - '
					.' <a href="'.site_url('tshirtecommerce/design.php?idea=1&key='.$product_id).'&view=right" target="_blank" title="'.$lang['order_click_to_view_design'].'"><strong>'.$lang['right'].'</strong></a>'
				. '</p>';
		}
	}
	echo '</div>';
