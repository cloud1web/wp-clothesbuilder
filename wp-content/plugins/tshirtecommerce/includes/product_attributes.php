...
{
	if ( isset($_POST['attribute']) && isset($_POST['product_id']) && isset($_POST['is_page_detail']) )
	{
		$data 				= array();
		...
		if ($update_price == true)
		{
			...
			if (isset($prices->sale))
			{
				...
				$price = str_replace($price_decimal, '.', $prices->sale);
...
function designer_product_button($values)
{
	...
	{		
		...
		if (isset($data->show_attribute) && $data->show_attribute == 1)
			echo '<style>.post-'.$product->id.' .add_to_cart_button{display: none !important;}</style>';
	}
}
