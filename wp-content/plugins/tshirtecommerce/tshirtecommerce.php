...
function tshirtecommerce_plugin_activate() {
	add_option( 'Activated_Plugin', 'tshirtecommerce' );
			...
			if (count($data))
			{
				...
				foreach($data as $key => $value)
				{
					...
					if ($allow_update == true)
					{
						...
						echo			'<div style="float:right;padding-top: 5px;padding-right: 10px;line-height: 22px;">'
											.'<a class="button button-primary button-small" href="?page=online_designer_update&action=update&file='.$value->version.'">Update</a>'
											.' or <a target="_blank" href="'.$url.$value->version.'">download</a>'
										.'</div>';
					}
	...
	<form name="form1" method="post" action="">
		...
		<table class="form-table">
			<tbody>
				...
				<tr valign="top">
					<th scope="row" class="titledesc"><?php _e("Class name of button design", 'menu-test'); ?> </th>
					<td class="forminp-text">
						<input type="text" name="<?php echo $data_field_extra_class; ?>" value="<?php echo $opt_val['btn-extra_class']; ?>" />					
					</td>
				</tr>
...
function design_button() {
	...
	$show_butoon = apply_filters( 'tshirtecommerce_design_button', true, $product_id );
	if ($link != '')
	{
		...
		if (isset($price[0]) && $price[0] > 0)
		{
			...
			if ($show_butoon == true)
			{
				...
				else
				{
					...
					{
						$link = add_query_arg( array('cart_id'=>$array[0]), $page );
						echo '<a class="button e-custom-product '.$opt_val['btn-extra_class'].'" onclick="return loadProductDesign(this);" href="'.$link.'">'.$opt_val['btn-custom'].'</a>';	
					}
				}
			}
...
function tshirtecommerce_func( $atts )
{
	...
	else
	{
		...
		if (isset($_GET['design']))
		{
			...
			{
				if ($designs[0] == 'cart')
					$url = site_url('tshirtecommerce/index.php?product='.$designs[2].'&color='.$designs[3].'&cart_id='.$designs[1].'&parent='.$designs[4]);
				else
		...
		{
			$url = site_url('tshirtecommerce/index.php?product='.$array[2].'&color='.$array[3].'&user='.$array[0].'&id='.$array[1].'&parent='.$product_id);
		}
		else
		{
			$url = site_url('tshirtecommerce/index.php?product='.$product_design.'&parent='.$product_id);
...
function render_meta_on_cart_item( $title = null, $cart_item = null, $cart_item_key = null ) {
	if( $cart_item_key && is_cart() ) {
		...
		if ($data != null && count($data) > 0 && isset($data['design_id']) && $data['design_id'] != '')
		{
			...
			{
				echo '<dt>'.$lang['designer_color'].': </dt>';
		...
		if (isset($data['images']))
		{
			...
			if (count($images))
			{
				...
				foreach($images as $view => $image)
				{
					...
					echo '<br /><a target="_blank" href="'.site_url('tshirtecommerce/design.php?key='.$data['design_id'].'&view='.$view.'&session_id='.session_id()).'">'.$lang['order_download_design'].'</a>';
					echo '</td>';
				}
				echo '</tr></table>';
			...
			else
			{
				...
				echo '<p>'
					...
					.' <a href="'.site_url('tshirtecommerce/design.php?key='.$data['design_id']).'&view=front'.$session_id.'" target="_blank" title="'.$lang['order_click_to_view_design'].'"><strong>'.$lang['front'].'</strong></a> - '
					.' <a href="'.site_url('tshirtecommerce/design.php?key='.$data['design_id']).'&view=back'.$session_id.'" target="_blank" title="'.$lang['order_click_to_view_design'].'"><strong>'.$lang['back'].'</strong></a> - '
					.' <a href="'.site_url('tshirtecommerce/design.php?key='.$data['design_id']).'&view=left'.$session_id.'" target="_blank" title="'.$lang['order_click_to_view_design'].'"><strong>'.$lang['left'].'</strong></a> - '
					.' <a href="'.site_url('tshirtecommerce/design.php?key='.$data['design_id']).'&view=right'.$session_id.'" target="_blank" title="'.$lang['order_click_to_view_design'].'"><strong>'.$lang['right'].'</strong></a>'
				. '</p>';
			}
			$html = '';
	...
	else
	{
		...
		if ($product_id != '')
		{
			...
			echo '<p>'
					...
					.' <a href="'.site_url('tshirtecommerce/design.php?idea=1&key='.$product_id).'&view=front'.$session_id.'" target="_blank" title="'.$lang['order_click_to_view_design'].'"><strong>'.$lang['front'].'</strong></a> - '
					.' <a href="'.site_url('tshirtecommerce/design.php?idea=1&key='.$product_id).'&view=back'.$session_id.'" target="_blank" title="'.$lang['order_click_to_view_design'].'"><strong>'.$lang['back'].'</strong></a> - '
					.' <a href="'.site_url('tshirtecommerce/design.php?idea=1&key='.$product_id).'&view=left'.$session_id.'" target="_blank" title="'.$lang['order_click_to_view_design'].'"><strong>'.$lang['left'].'</strong></a> - '
					.' <a href="'.site_url('tshirtecommerce/design.php?idea=1&key='.$product_id).'&view=right'.$session_id.'" target="_blank" title="'.$lang['order_click_to_view_design'].'"><strong>'.$lang['right'].'</strong></a>'
				. '</p>';
		}
	}
}
