		...
		if($available_fields){
			...
			foreach($available_fields as $available_field){
				...
				$html = '<div class="form-field _product_link_field" style="float: left; margin: 10px 10px 0px;">'					
					. 	'<div id="e_tshirt_load_blank" class="btn-group">'
					.		'<button type="button" class="btn btn-default">Change Product Design</button>'
					...
					.		'</ul>'
					.	'</div> '
					. 	' <div id="e_tshirt_load_design_temp" class="btn-group">'
					.		'<button type="button" class="btn btn-primary">Design Template</button>'
	...
	public function wc_product_data_options_fields($field)
	{
		...
		switch($fieldtype)
		{
			...
			case 'image':
				...
				echo 	'<input type="hidden" class="' . esc_attr( $field['class'] ) . '" name="' . esc_attr( $field['id'] ) . '" id="' . esc_attr( $field['id'] ) . '" value="' . esc_attr( $field['value'] ) .  '" /> ';
				echo 	'<script type="text/javascript">var site_e_url = "'.get_site_url().'"; var session_id = "'.session_id().'";</script>';
				echo 	'<div id="add_designer_product"></div>';
				echo '</div>';
				break;
		}
	}
