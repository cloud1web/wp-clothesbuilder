var productElm = {
	...
	editElm: function(id){
		...
		var a = jQuery('.product-elm-colors a');
		var div = jQuery('.product-elm-images');
		jQuery.each(colors, function(i, color){
			if(typeof color.color != 'undefined')
			{
				if(jQuery(a[i]).length > 0)
				{
					jQuery(a[i]).addClass('active');
				}
			}
			else if(typeof color.img != 'undefined')
			{
				div.append('<span class="product-img"><span onclick="productElm.removeImages(this)" class="btn-remove">&times;</span><img src="'+color.img+'" alt="" width="50" height="50"></span>');
			}
		});		
	...
	addImages: function(images){
		if(images.length > 0)
		{
			...
			{
				var src = images[i];
				div.append('<span class="product-img"><span onclick="productElm.removeImages(this)" class="btn-remove">&times;</span><img src="'+src+'" alt="" width="50" height="50"></span>');
			}
		}
