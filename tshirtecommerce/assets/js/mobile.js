	...
	areaDesign: function(zoomItem){
		...
		if (width_site < 500)
		{
			...
			if (changed == true)
			{
				...
				jQuery('body').append('<style>#app-wrap.view-mobile .labView, #app-wrap.view-mobile .product-design img{max-width:'+width_site+'px;max-height:'+width_site+'px;}</style>');
			}
		}
	},
...
jQuery(document).on('changeView.product.design', function(event, e){
	design.mobile.areaDesign();
	var postions = {'0':'front', '1':'back', '2':'left', '3':'right'};
	var idx      = jQuery('#product-thumbs a').index(jQuery(e));
	var pos      = postions[idx];
	jQuery('.labView').removeClass('active');
	jQuery('#view-' + pos).addClass('active');
